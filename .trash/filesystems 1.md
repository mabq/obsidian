...
The very first thing to understand is that there are many different types of filesystems, each has its own advantages, disadvantages, and unique idiosyncrasies.

#### Disk-based (local) filesystems

Individual drive partitions can be set up using one of the many different available file systems.
...
> [!important]
> The Linux directory tree is the result of mounting different filesystems (of different types) into different mount points.
...
UEFI firmware directly reads the GPT partition table, finds the EFI System Partition (ESP), and executes the bootloader (like GRUB, systemd-boot, or rEFInd) stored inside it. The firmware does not need the main OS to be running, nor does it care where Linux mounts that partition later.

So why do we mount it to `/boot` (or `/boot/efi`) once Linux is actually up and running?

Well, simply because the bootloader and kernel aren't static, they need to be updated from time to time.

By mounting the boot partition to `/boot` (or `/boot/efi`), the package manager can automatically drop the new kernel and bootloader files directly onto the partition that UEFI reads during the next boot. If the boot partition wasn't mounted, your system update would write the new kernel to the main system disk inside a plain folder named `/boot` (not shadowed). But when you rebooted, the UEFI firmware would still load the old kernel living on the unmounted boot partition.
